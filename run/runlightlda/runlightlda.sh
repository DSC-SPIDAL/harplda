...
runlightlda()
{
    ...
    $bindir/lightlda -num_vocabs $num_vocabs -num_topics $topic -num_iterations $iter -alpha $alpha -beta $beta -mh_steps $mh_step -num_local_workers $threads -num_blocks 1 -max_num_document $max_doc -input_dir $datadir -data_capacity $data_cap  | tee $logfile
