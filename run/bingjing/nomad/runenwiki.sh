...
run_nomad()
{
    ...
    mpirun -f juliet-hsw72-10.ip -n $2 `pwd`/bin/f+nomad-lda -T $timeout -k $K -a $alpha -b $beta -t $1 -n $3 $homedir/$dataset 2>&1 | tee nomadlda_$1_$2_$3_$dataset-$K-$alpha-$beta_$4.log
}
...
runid=$1
appname="$dataset$K-10x30-hsw72-$network-$runid"
