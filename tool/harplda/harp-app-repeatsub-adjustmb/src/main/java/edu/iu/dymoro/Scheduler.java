...
public class Scheduler<D, S extends Simple, T extends MPTask<D, S>> {
  ...
  private final int SUBMISSION_LEVEL = 5;
  private int MAXSUBMISSION = 5;
  ...
  public void schedule(List<Partition<S>>[] hMap,
    ...
    while (numFreeRows > 0 && numFreeCols > 0) {
      ...
      submissionMap[split.row][split.col] >>= SUBMISSION_LEVEL;
      rowCount[split.row]++;
      colCount[split.col]++;
      ...
      if (isRunning.get()) {
        ...
        if (freeRowID != -1 && numFreeCols > 0) {
          ...
          for (int i = 0; i < numFreeCols; i++) {
            ...
            if (submissionMap[freeRowID][freeCol[i]] > 1) {
              count += submissionMap[freeRowID][freeCol[i]] ;
              if (random.nextFloat() * count < 
                submissionMap[freeRowID][freeCol[i]] ) {
                selectedColIndex = i;
              }
            }
          }
          if (count > 0) {
            ...
            submissionMap[s.row][s.col] >>= SUBMISSION_LEVEL;
            rowCount[s.row]++;
            colCount[s.col]++;
        ...
        if (freeColID != -1 && numFreeRows > 0) {
          ...
          for (int i = 0; i < numFreeRows; i++) {
            ...
            if (submissionMap[freeRow[i]][freeColID] > 1) {
              count += submissionMap[freeRow[i]][freeColID];
              if (random.nextFloat() * count < 
                  submissionMap[freeRow[i]][freeColID]) {
                selectedRowIndex = i;
              }
            }
          }
    ...
    ////for (int i = 0; i < numRowSplits; i++) {
    ////  rowmap += rowCount[i] + " ";
    ////  allRowCount += rowCount[i];
    ////}
    ////LOG.info("allRowCount:" + allRowCount + ": " + rowmap);

    ////allRowCount = 0;
    ////rowmap = "";  
    ////for (int i = 0; i < numColSplits; i++) {
    ////  rowmap += colCount[i] + " ";
    ////  allRowCount += colCount[i];
    ////}
    ////LOG.info("allRowCount:" + allRowCount + ": " + rowmap);
    ...
    for (int i = 0; i < numRowSplits; i++) {
      for (int j = 0; j < numColSplits; j++) {
        ...
        //if (submissionMap[i][j]) {
          ...
          submissionMap[i][j] = 1<<(SUBMISSION_LEVEL*MAXSUBMISSION);
        //}
      }
    }
  }
