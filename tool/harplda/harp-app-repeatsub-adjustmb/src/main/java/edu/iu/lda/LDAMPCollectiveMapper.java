...
public class LDAMPCollectiveMapper extends
  ...
  protected void setup(Context context) {
    ...
    LOG.info("enableTuning\\Time\\Bounds\\MaxSubmission "
      + enableTuning + "\\" + time + "\\"
      + minBound + "\\" + maxBound + "\\" + maxSubmission);
  ...
  private long adjustMiniBatch(int selfID,
    ...
    if (newMinibatch != miniBatch) {
      LOG.info("Total trained tokens: "
        ...
        + " new timer: " + newMinibatch + " old timer: " 
        + miniBatch + " o/u/min/max: " + overTrain + '/' 
        + '/' + underTrain + '/' + minBound + '/' + 
        maxBound);
    }
    return newMinibatch;
