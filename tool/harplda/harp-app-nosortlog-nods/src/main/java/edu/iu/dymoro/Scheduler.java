...
public class Scheduler<D, S extends Simple, T extends MPTask<D, S>> {
  ...
  public void schedule(List<Partition<S>>[] hMap,
    ...
    //shift version adopts a simple scheduler strategy
    //0. blocks split ratio == 1, init with diagonal
    int rowIndex = 0;
    int colIndex = 0;
    while (colIndex < numColSplits) {
  ...
  public void schedule_old(List<Partition<S>>[] hMap,
    boolean record) {
    init(hMap);
    for (int i = 0; i < numRowSplits; i++) {
