...
void WarpLDA<MH>::initialize()
{
	...
	shuffle->VisitByV([&](TVID v, TDegree N, const TUID* lnks, TData* data)
	{
		...
		for (TDegree i = 0; i < N; i++)
		{
			...
			data[i].oldk = k;
            for (unsigned mh=0; mh<this->mh_step; mh++)
                data[i].newk[mh] = k;
			++ck_new[k];
		}
	});
...
template<unsigned MH>
template <bool testMode>
void WarpLDA<MH>::accept_d_propose_w(int iterid)
{
    #pragma omp parallel
    {
...
template <unsigned MH>
void WarpLDA<MH>::estimate(int _K, float _alpha, float _beta, int _niter, int _perperplexity_interval, unsigned _mh_step)
{
    this->K = _K;
    this->alpha = _alpha;
    this->beta = _beta;
    this->niter = _niter;
    this->mh_step = _mh_step;
    initialize();

    printf("Start: K=%d, alpha=%f, beta=%f, niter=%d, mhstep=%d\n",_K, _alpha, _beta, _niter, _mh_step);
	fflush(stdout);
