...
void sampler_fun(int thread_id, comm_space_t &space) {//{{{
	...
	while(true) {
        ...
        //add threadlog
        //double _start_time = omp_get_wtime();

        //double _start_time = (double)clock();

        //struct timespec ts1, ts2;
        //clock_gettime(CLOCK_MONOTONIC, &ts1);

        //uint64_t _start_time = rdtsc();
		...
		if(training.word_ptr[cur_word] != training.word_ptr[cur_word+1]) { // {{{
            ...
            uint64_t duration_time = rdtsc() - _start_time;

		} // }}}

        //add of training part
        //double duration_time = omp_get_wtime() - _start_time;
        
        //double duration_time = (double)clock() - _start_time;

        //clock_gettime(CLOCK_MONOTONIC, &ts2);
        //double duration_time = ts2.tv_sec - ts1.tv_sec;
